...
Public Class TestIssuanceDetail
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Session("strProdOrTestEnvironment") = System.Configuration.ConfigurationManager.AppSettings("DBInstance").ToString()
            Session("strProdOrTestURL") = System.Configuration.ConfigurationManager.AppSettings("prodOrTestURL").ToString()


            ''***********************************************
            ''Code Below overrides the Page Title at the content level 
            ''***********************************************
            Dim mpPageTitle As Label
            mpPageTitle = CType(Master.FindControl("pwPageTitle"), Label)
            If Not mpPageTitle Is Nothing Then
                Select Case ViewState("pReqCategory")
                    Case 1
                        mpPageTitle.Text = "Product Innovation"
                        ViewState("ReqCategorDesc") = "Product Innovation"
                    Case 2
                        mpPageTitle.Text = "Current Mass Production Part"
                        ViewState("ReqCategorDesc") = "Current Mass Production Part"
                    Case 3
                        mpPageTitle.Text = "Consultation"
                        ViewState("ReqCategorDesc") = "Current Mass Production Part"
                    Case 4
                        mpPageTitle.Text = "New Program Launch"
                        ViewState("ReqCategorDesc") = "New Program Launch"
                End Select
                mpPageTitle.Visible = True
            End If
            ...
            ''**************************************************
            Dim ctl As Control = Master.FindControl("lblOtherSiteNode")
            If ctl IsNot Nothing Then
                Dim lbl As Label = CType(ctl, Label)
                Select Case ViewState("pReqCategory")
                    Case 1
                        lbl.Text = "R&D Lab > <a href='TestIssuanceList.aspx'>Test Issuance Search</a> > Product Innovation"
                    Case 2
                        lbl.Text = "R&D Lab > <a href='TestIssuanceList.aspx'>Test Issuance Search</a> > Current Mass Production Part"
                    Case 3
                        lbl.Text = "R&D Lab > <a href='TestIssuanceList.aspx'>Test Issuance Search</a> > Consultation"
                    Case 4
                        lbl.Text = "R&D Lab > <a href='TestIssuanceList.aspx'>Test Issuance Search</a> > New Program Launch"
            ...
            ' ''******************************************
            ctl = Master.FindControl("RnDExtender")
            If ctl IsNot Nothing Then
                Dim cpe As AjaxControlToolkit.CollapsiblePanelExtender = CType(ctl, AjaxControlToolkit.CollapsiblePanelExtender)
                cpe.Collapsed = False
            End If
...
#Region "Form Level Security using Roles &/or Subscriptions"
    Protected Sub CheckRights()
        ...
        Try
            ...
            dsTeamMember = SecurityModule.GetTeamMember(Nothing, strFullName, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
            ''dsTeamMember = SecurityModule.GetTeamMember(Nothing, "Demo.Demo", Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
...
#Region "General"
    ...
    Protected Sub ddCustomer_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddCustomer.SelectedIndexChanged
        ProgramCollection()
    End Sub

    Private Sub ProgramCollection()
        Dim ds As DataSet = New DataSet
        ' '' ''bind existing data to drop down Customer control for selection criteria for search
        ds = commonFunctions.GetProgram("", "", IIf(ddCustomer.SelectedValue = Nothing, "", ddCustomer.SelectedValue))
        If (ds.Tables.Item(0).Rows.Count > 0) Then
        ...
        Try
            ...
            If (ds.Tables.Item(0).Rows.Count > 0) Then
                ...
                If Not IsDBNull(ds.Tables(0).Rows(0).Item("ProjectID")) Then
                    ...
                    hlnkACProjNo.ForeColor = Drawing.Color.Blue
                Else
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If imgBtn IsNot Nothing Then
                ...
                If db.CommandName = "Delete" Then
                    Dim price As RnDDAL.TestIssuance_CustomerPartRow = CType(CType(e.Row.DataItem, System.Data.DataRowView).Row, RnDDAL.TestIssuance_CustomerPartRow)
    ...
    Protected Sub gvTMAssignments_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvTMAssignments.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If imgBtn IsNot Nothing Then
                ...
                ' Get information about the product bound to the row
                If db.CommandName = "Delete" Then
                    Dim price As RnDDAL.TestIssuance_AssignmentsRow = CType(CType(e.Row.DataItem, System.Data.DataRowView).Row, RnDDAL.TestIssuance_AssignmentsRow)
    ...
    Protected Sub gvTestReport_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvTestReport.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If imgBtn IsNot Nothing Then
                ...
                ' Get information about the product bound to the row
                If db.CommandName = "Delete" Then
                    Dim price As RnDDAL.TestIssuance_TestReportRow = CType(CType(e.Row.DataItem, System.Data.DataRowView).Row, RnDDAL.TestIssuance_TestReportRow)
...
#Region "Insert Empty GridView Work-Around for gvTMAssignments"
    ...
    Protected Sub odsTMAssignments_Selected(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ObjectDataSourceStatusEventArgs) Handles odsTMAssignments.Selected
        ...
        Dim dt As RnDDAL.TestIssuance_AssignmentsDataTable = CType(e.ReturnValue, RnDDAL.TestIssuance_AssignmentsDataTable)
...
#Region "Test Report"
    ...
    Public Function DisplayImage(ByVal EncodeType As String) As String
        ...
        ElseIf EncodeType = "application/vnd.ms-excel" Then
            strReturn = "~/images/Extenstions/xls.jpg"
        ElseIf EncodeType = "application/pdf" Then
            strReturn = "~/images/Extenstions/pdf.jpg"
        ElseIf EncodeType = "application/msword" Then
            strReturn = "~/images/Extenstions/doc.jpg"
...
#Region "Email Notification"
    Public Function EmailBody(ByVal MyMessage As MailMessage) As String
        MyMessage.Body &= "<table width='60%' style='font-size:smaller; font-family:Tahoma; border: 1px solid #D0D0BF; border-left-style:dotted; border-right-style:dotted; border-collapse: collapse;'>"
        MyMessage.Body &= "<tr bgcolor='#EBEBEB' style='border-color:#EBEBEB;'>"
        MyMessage.Body &= "<td width='388'><strong>Sample Product Description</strong></td>"
        MyMessage.Body &= "<td width='423'><strong>Sample Issuer </strong></td>"
        MyMessage.Body &= "</tr>"
        MyMessage.Body &= "<tr style='border-color:white;'>"
        MyMessage.Body &= "<td height='25'>" & txtSampleProdDesc.Text & "</td>"
        MyMessage.Body &= "<td height='25'>" & ddSampleIssuer.SelectedItem.Text & "</td>"
        MyMessage.Body &= "</tr>"
        MyMessage.Body &= "<tr bgcolor='#EBEBEB' style='border-color:white;'>"
        MyMessage.Body &= "<td><strong>UGN Location </strong></td>"
        MyMessage.Body &= "<td><strong>Commodity</strong></td>"
        MyMessage.Body &= "</tr>"
        MyMessage.Body &= "<tr>"
        MyMessage.Body &= "<td height='25'>" & ddUGNFacility.SelectedItem.Text & "</td>"
        MyMessage.Body &= "<td height='25'>" & ddCommodity.SelectedItem.Text & "</td>"
        MyMessage.Body &= "</tr>"
        MyMessage.Body &= "<tr>"
        ...
        ''***************************************************
        MyMessage.Body &= "<table width='100%' border='0' style='font-size:smaller; font-family:Tahoma;'>"
        MyMessage.Body &= "<tr bgcolor='#EBEBEB' style='border-color:#EBEBEB'>"
        ...
        MyMessage.Body &= "<td><strong>Design Level </strong></td>"
        MyMessage.Body &= "</tr>"
        Dim dsCP As DataSet
        dsCP = RnDModule.GetTestIssuanceCustomerPart(ViewState("pReqID"))
        ...
        MyMessage.Body &= "<td colspan='2'><strong>Description of Required Testing</strong></td>"
        MyMessage.Body &= "</tr>"
        MyMessage.Body &= "<tr style='border-color:white;'>"
        MyMessage.Body &= "<td colspan='2'>" & txtDescReqTesting.Text & "</td>"
        MyMessage.Body &= "</tr>"
        MyMessage.Body &= "</Table>"
    ...
    Sub btnSubmit1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit1.Click
        Try
            ...
            Else
                CurrentEmpEmail = "Notifications@ugn.com"
                EmailFrom = "Notifications@ugn.com"
            End If
                ...
                If ViewState("pReqID") <> 0 Then
                    ...
                    Else
                        ...
                        If EmailTO <> Nothing Then
                            ...
                            MyMessage.Body &= "<p><Please review Cust-TI-Req-" & ViewState("pReqID") & " for " & ViewState("ReqCategorDesc")
                            MyMessage.Body &= " and provide results by " & txtTestCmpltDt.Text
                            MyMessage.Body &= ". <a href='" & Session("strProdOrTestURL") & "/RnD/TestIssuanceDetail.aspx?pReqID=" & ViewState("pReqID") & "&pReqCategory=" & ViewState("pReqCategory") & "'>Click here</a> to access the record.</p>"
                            MyMessage.Body &= "</font>"
                            ...
                            If InStr(Session("strProdOrTestEnvironment"), "Test_", CompareMethod.Text) > 0 Then
                                MyMessage.Body &= "<p>EmailTO: " & EmailTO & "</p>"
                                MyMessage.Body &= "<p>EmailCC: " & EmailCC & "</p>"
                                EmailFrom = "Notifications@ugn.com"
                                EmailTO = "ltorres@ugn.com"
                                EmailCC = "ltorres@ugn.com"
                            End If
                ...
                If ViewState("pReqID") <> 0 Then
                    ...
                    Else
                        ...
                        If EmailTO <> Nothing Then
                            ...
                            MyMessage.Body = "<font size='2' face='Tahoma'>" & EmpName
                            MyMessage.Body &= "<p>Cust-TI-Req-" & ViewState("pReqID") & " for " & ViewState("ReqCategorDesc")
            ...
            If CurrentEmpEmail <> Nothing Then
                If ViewState("pReqID") <> 0 Then
                    ...
                    Else
                        ...
                        ''*******************************
                        RnDds = SecurityModule.GetTeamMember(ddSampleIssuer.SelectedValue, Nothing, Nothing, Nothing, Nothing, Nothing, True, Nothing, Nothing)
                        ''Check that the recipient(s) is a valid Team Member
                        If RnDds.Tables.Count > 0 And (RnDds.Tables.Item(0).Rows.Count > 0) Then
                            For i = 0 To RnDds.Tables.Item(0).Rows.Count - 1
